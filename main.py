...
class TelegramAlbumTransfer:
    ...
    async def process_with_three_queues(self):
        # Correção: ordenar também pelo menor message_id das mídias do álbum para garantir desempate por segundo
        sorted_albums = sorted(
            self.albums.values(),
            key=lambda x: (x.date, min(m.message_id for m in x.medias))
        )
    ...
    async def scan_messages_chronological(self):
        ...
        # Correção: ordenar por data e id para garantir precisão até o segundo e desempate
    ...
    async def process_messages_for_albums(self, messages: List[Message]):
        ...
        for grouped_id, medias in album_groups.items():
            if len(medias) >= 2:
                # Correção: ordenar mídias por data e id para garantir ordem precisa dentro do álbum
                medias.sort(key=lambda x: (x.date, x.message_id))
        ...
        # Garante que a propriedade exista antes de usar
        if hasattr(self, "single_medias"):
